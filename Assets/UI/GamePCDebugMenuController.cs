...
public class GamePCDebugMenuController : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        // Register game mode button callbacks
        ...
        if (MQTTManager.Instance != null)
        {
            ...
            MQTTManager.Instance.OnGameModeReceived += OnGameModeReceived;
        }
    ...
    private void OnMultiFingerTap()
    {
        ...
        Debug.Log($"[Game PC] Multi-finger tap detected! Count: {currentTapCount}/{requiredTaps}");
    ...
    {
        UpdateConnectionStatus();
        Debug.LogError($"[Game PC] MQTT Connection failed: {error}");
    }

    private void OnGameModeReceived(GameModeMessage msg)
    {
        Debug.Log($"[Game PC] Received game mode from tablet: {msg.gameMode}");
        ...
        {
            selectedModeLabel.text = $"Selected: {currentGameMode.ToUpper()}";
        }
    ...
    private void LoadSettings()
    {
        ...
        {
            Debug.LogWarning("[Game PC Debug Menu] UI elements not found yet, skipping LoadSettings");
            return;
        }
        ...
        if (MQTTManager.Instance != null)
        {
            ...
            Debug.Log($"[Game PC Debug Menu] Loaded current settings - Broker: {MQTTManager.Instance.BrokerAddress}:{MQTTManager.Instance.BrokerPort}, Station ID: {MQTTManager.Instance.StationId}");
        }
        else
        {
            // Fallback to PlayerPrefs if MQTTManager not found
            brokerAddressInput.value = PlayerPrefs.GetString("GamePC_MQTT_BrokerAddress", "localhost");
            brokerPortInput.value = PlayerPrefs.GetInt("GamePC_MQTT_BrokerPort", 1883).ToString();
            stationIdInput.value = PlayerPrefs.GetInt("GamePC_MQTT_StationId", 1).ToString();

            Debug.Log("[Game PC Debug Menu] MQTTManager not found, loaded from PlayerPrefs");
        }

        // Load other settings from PlayerPrefs
        if (usernameInput != null) usernameInput.value = PlayerPrefs.GetString("GamePC_MQTT_Username", "");
        if (passwordInput != null) passwordInput.value = PlayerPrefs.GetString("GamePC_MQTT_Password", "");

        // Load game mode from PlayerPrefs
        currentGameMode = PlayerPrefs.GetString("GamePC_GameMode", "rowing");
        if (selectedModeLabel != null) selectedModeLabel.text = $"Selected: {currentGameMode.ToUpper()}";
    ...
    {
        // Update IdleController content
        if (idleController != null)
        {
            idleController.SetGameMode(mode);
            Debug.Log($"[Game PC Debug Menu] Applied game mode '{mode}' to IdleController");
        }
        else
        {
            Debug.LogWarning("[Game PC Debug Menu] IdleController reference is NULL! Assign it in the inspector.");
        }

        // Update MainGameManager mesh generators
        if (gameManager != null)
        {
    ...
    {
        PlayerPrefs.SetString("GamePC_MQTT_BrokerAddress", brokerAddressInput.value);
        if (int.TryParse(brokerPortInput.value, out int port))
        {
            PlayerPrefs.SetInt("GamePC_MQTT_BrokerPort", port);
        }
        if (int.TryParse(stationIdInput.value, out int stationId))
        {
            PlayerPrefs.SetInt("GamePC_MQTT_StationId", stationId);
        }
    ...
    private void SelectGameMode(string mode)
    {
        ...
        if (selectedModeLabel != null)
        {
            selectedModeLabel.text = $"Selected: {mode.ToUpper()}";
        }
        ...
        Debug.Log($"[Game PC Debug Menu] Game mode changed to: {mode}");
    }

    private void UpdateGameModeButtonStyles()
    {
        // Reset all buttons
        Color defaultColor = new Color(0.24f, 0.24f, 0.24f);
        Color selectedColor = new Color(1f, 0.55f, 0f); // Orange

        if (rowingButton != null)
            rowingButton.style.backgroundColor = currentGameMode == "rowing" ? selectedColor : defaultColor;
        if (runningButton != null)
            runningButton.style.backgroundColor = currentGameMode == "running" ? selectedColor : defaultColor;
        if (cyclingButton != null)
            cyclingButton.style.backgroundColor = currentGameMode == "cycling" ? selectedColor : defaultColor;
    }
